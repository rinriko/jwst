...
def convert_time_units(mjd_times, unit):
    ...
    if unit == 'second':
        converted_times = (time_sorted.mjd -
                           time_sorted.mjd.min()) * 86400.0  # seconds
    elif unit == 'minute':
        converted_times = (time_sorted.mjd -
                           time_sorted.mjd.min()) * 1440.0  # minutes
    elif unit == 'hour':
        converted_times = (time_sorted.mjd -
                           time_sorted.mjd.min()) * 24.0  # hours
    elif unit == 'day':
...
# CORRUPT_CSV_FOLDER = r"T:\JWST-Timing\Jay\Corrupted FrameIN30\CSV"
CORRUPT_CSV_FOLDER = r"T:\JWST-Timing\Jay\Corrupted FrameIN30\CSVOutput"

csv_files = glob.glob(os.path.join(CORRUPT_CSV_FOLDER, "*.csv"))
...
for f in csv_files:
    ...
    match = re.search(r"frame_(\d+)\.csv$", name)

    if match is None:
        print(f"Skip non-frame CSV: {name}")
        continue

    CORRUPT_FRAMES.append(int(match.group(1)))

CORRUPT_FRAMES = sorted(CORRUPT_FRAMES)

print(CORRUPT_FRAMES)
print("Total corrupt frames:", len(CORRUPT_FRAMES))
...
for epoch in ["epoch1"]:
    ...
    # for wave_type in ["lw", "sw"]:
        for rIn in range(10, 12, 4):
            # for rIn in range(10,32,4):
            for rOut in range(30, 35, 10):
                ...
                if rOut > rIn:
                    ...
                    path = "T:\JWST-Timing\MAST_2023-10-07T0548\JWST"
                    ...
                    # first extension is the PSF oversampled by 4x relative to detector sampling
                    F070W_PSF_oversampled = F070W_PSF[0]
                    # second extension is the PSF at detector sampling
                    F070W_PSF_detsampled = F070W_PSF[1]

                    F277W_PSF = fits.open(F277W_canned_PSF_file)

                    # first extension is the PSF oversampled by 4x relative to detector sampling
                    F277W_PSF_oversampled = F277W_PSF[0]
                    # second extension is the PSF at detector sampling
                    F277W_PSF_detsampled = F277W_PSF[1]
                    ...
                    sw_files = sorted(glob.glob(os.path.join(
                        path, "jw01666001001*_nrcb1", "*crfints.fits")))
                    lw_files = sorted(glob.glob(os.path.join(
                        path, "jw01666001001*_nrcblong", "*crfints.fits")))
                    if epoch == "epoch2":
                        sw_files = sorted(glob.glob(os.path.join(
                            path, "jw01666003001*_nrcb1", "*crfints.fits")))
                        lw_files = sorted(glob.glob(os.path.join(
                            path, "jw01666003001*_nrcblong", "*crfints.fits")))
                    ...
                    for x in range(len(fileList)):
                        ...
                        # detector name header for LW doesn't actually match with what is accepted by WebbPSF
                        detector = {"NRCB1": "NRCB1", "NRCBLONG": "NRCB5"}
                        # Starting pixel in axis 1 direction
                        det_x0 = f[0].header['SUBSTRT1']
                        # Starting pixel in axis 2 direction
                        det_y0 = f[0].header['SUBSTRT2']
                        # subarray size in pixels, x direction
                        subsize1 = f[0].header['SUBSIZE1']
                        # subarray size in pixels, y direction
                        subsize2 = f[0].header['SUBSIZE2']
                        ...
                        # I've minused 1 from each coord as coords are zero indexed in python but 1 indexed in ds9
                        centers = {"F070W": (83, 85), "F277W": (
                            86, 76), "F322W2": (86, 76)}
                        # we are going to want to do aperture photometry as well
                        r = {"F070W": 3, "F277W": 2, "F322W2": 3}
                        ...
                        # read in the data
                        # pixel values, in units of surface brightness, for each integration
                        data = f["SCI"]
                        # uncertainty estimates for each pixel, for each integration
                        err = f["ERR"]
                        # DQ flags for each pixel, for each integration
                        dq = f["DQ"]
                        
                        original_data = data.data.copy()

                        # ---------- replace one FITS frame with corrupted CSV ----------
                        CORRUPT_FITS_NAME = "jw01666001001_02102_00001-seg001_nrcb1_o001_crfints.fits"
                        CORRUPT_CSV_FOLDER = r"T:\JWST-Timing\Jay\Corrupted FrameIN30\CSV"

                        if os.path.basename(filename) == CORRUPT_FITS_NAME:
                            for frame_number in range(1, data.data.shape[0] + 1):
                                csv_path = os.path.join(
                                    CORRUPT_CSV_FOLDER,
                                    f"frame_{frame_number:06d}.csv"
                                )

                                if not os.path.exists(csv_path):
                                    continue

                                corrupt_frame = np.loadtxt(
                                    csv_path, delimiter=",")
                                frame_index = frame_number - 1

                                if corrupt_frame.shape != data.data[frame_index].shape:
                                    raise ValueError(
                                        f"Shape mismatch for frame {frame_number}: "
                                        f"CSV {corrupt_frame.shape}, FITS {data.data[frame_index].shape}"
                                    )

                                data.data[frame_index] = corrupt_frame
                                print(
                                    f"Replaced frame {frame_number} using {csv_path}")
                        # -------------------------------------------------------------
                        ...
                        ListOfFrames = range(istart+1, iend+1)
                        ListOfFrames = CORRUPT_FRAMES
                        if os.path.basename(filename) == CORRUPT_FITS_NAME:
                            ListOfFrames = CORRUPT_FRAMES
                        valid_indices = [
                            frame_number - 1
                            for frame_number in ListOfFrames
                            if 0 <= frame_number - 1 < data.data.shape[0]
                        ]
                        meandata = np.nanmean(data.data[valid_indices], axis=0)

                        x0, y0 = centers[f[0].header["FILTER"]]

                        ap_centroid_mean = centroid_sources(
                            meandata, (x0,), (y0,), box_size=21, centroid_func=centroid_com)
                        ap_xout_mean = ap_centroid_mean[0][0]
                        ap_yout_mean = ap_centroid_mean[1][0]

                        print(filename)
                        print("mean centroid for aperture photometry:",
                              ap_xout_mean, ap_yout_mean)

                        # enables the PSF calculation to be at the center of the subarray
                        detector_position = (
                            det_x0 + ap_xout_mean, det_y0 + ap_yout_mean)
                        ...
                        psf = nc.psf_grid(
                            num_psfs=1, single_psf_centered=False, all_detectors=False)
                        ...
                        bg_aperture_average = CircularAnnulus(
                            ((ap_xout_mean, ap_yout_mean)), r_in=r_in, r_out=r_out)
                        aperture_average = CircularAperture(
                            ((ap_xout_mean, ap_yout_mean)), r=r[f[0].header["FILTER"]])
                        sigclip_average = SigmaClip(sigma=3.0, maxiters=10)
                        bgstats_average = ApertureStats(
                            meandata, bg_aperture_average, sigma_clip=sigclip_average)
                        ...
                        ap_out_average = aperture_photometry(
                            meandata, aperture_average)
                        counts_bkgsub_average = ap_out_average["aperture_sum"][0] - (
                            apphot_bg_average*npix_average)
                        ...
                        av_pos = Table(names=["x_0", "y_0", "flux_init"], data=[
                                       (ap_xout_mean,), (ap_yout_mean,), (counts_bkgsub_average,)])

                        # DAOGroup deprecated, replacing with SourceGrouper
                        ...
                        psf_result_average = photometry(
                            data=meandata-apphot_bg_average, init_params=av_pos)

                        psf_xout_mean = psf_result_average["x_fit"][0]
                        psf_yout_mean = psf_result_average["y_fit"][0]

                        print("mean centroid for psf photometry:",
                              psf_xout_mean, psf_yout_mean)

                        centroid_log.append({
                            "fits_file": os.path.basename(filename),
                            "ap_x": ap_xout_mean,
                            "ap_y": ap_yout_mean,
                            "psf_x": psf_xout_mean,
                            "psf_y": psf_yout_mean,
                        ...
                        # there is no source in first few frames, this *could* cause a problem. Include them for now.
                        istart = 0
                        iend = data.data.shape[0]
                        ListOfFrames = range(istart+1, iend+1)
                        ListOfFrames = CORRUPT_FRAMES
                        if os.path.basename(filename) == CORRUPT_FITS_NAME:
                            ListOfFrames = CORRUPT_FRAMES
                        for frame_number in ListOfFrames:
                            i = frame_number - 1  # convert FITS frame number to Python index

                            if i < 0 or i >= data.data.shape[0]:
                                print(f"Frame {frame_number} outside FITS range, skip")
                                continue
                        ...
                        #     print(filename, i)
                            ...
                            bgstats = ApertureStats(
                                data.data[i], bg_aperture, mask=dq.data[i] > 0, sigma_clip=sigclip)
                            apphot_bg = bgstats.median
                            apphot_bg_err = bgstats.std
                            npix = aperture.area_overlap(
                                data.data[i], mask=dq.data[i] > 0)
                            ...
                            # average bg per pixel
                            ap_out["apphot_bg"] = apphot_bg
                            # stdev of bg
                            ap_out["apphot_bg_err"] = apphot_bg_err
                            ap_out["npix"] = npix
                            ap_out["counts_bkgsub"] = ap_out["aperture_sum"] - \
                                (ap_out["apphot_bg"]*ap_out["npix"])
                            ap_out["counts_bkgsub_err"] = np.sqrt(
                                (ap_out["npix"] * ap_out["apphot_bg_err"]**2) + (ap_out["aperture_sum_err"]**2))
                            ...
                            if ap_out["counts_bkgsub"]/ap_out["counts_bkgsub_err"] < 3:
                                ...
                                pos = Table(names=["x_0", "y_0", "flux_init"], data=[
                                            (xout,), (yout,), (ap_out["counts_bkgsub"][0],)])
                                psf.x_0.fixed = True
                                psf.y_0.fixed = True
                            else:
                                if not psf_result:
                            ...
                            # make sure this is the best estimate of background for PSF
                            bkg = mmm_bkg(data.data[i])
                            ...
                            # used to be called "image", now it's "data", init_guesses is now init_params
                            result_tab = photometry(
                                data=data.data[i]-apphot_bg, init_params=pos, mask=dq.data[i] > 0)
                            ...
                            if np.abs(result_tab['x_fit'][0] - psf_xout_mean) > 2 or np.abs(result_tab['y_fit'][0] - psf_yout_mean) > 2:
                                pos = Table(names=["x_0", "y_0", "flux_init"], data=[
                                            (psf_xout_mean,), (psf_yout_mean,), (ap_out["counts_bkgsub"][0],)])
                                psf.x_0.fixed = True
                                psf.y_0.fixed = True
                                result_tab = photometry(
                                    data=data.data[i]-apphot_bg, init_params=pos, mask=dq.data[i] > 0)
                            ...
                            if psf.x_0.fixed:
                                # used to be called x_fit_unc
                                result_tab["x_err"] = 0.0
                            ...
                            # get_residual_image is now "make_residual_image"
                            residual_image = photometry.make_residual_image(
                                data.data[i], (fitbox, fitbox))
                            ...
                            flux_err = np.sqrt(
                                1 / nansumwrapper((psf_image[slice] ** 2 / err.data[i][slice] ** 2)))
                            # chisqb = (((im - flux * psf_image[slice]) / err.data[i][slice]) ** 2).sum()
                            chisqb = nansumwrapper(
                                (((im - flux * psf_image[slice]) / err.data[i][slice]) ** 2))

                            result_tab["chisq"] = chisq
                            result_tab["chisq_v2"] = chisqb
                            if psf.x_0.fixed:
                                # psf fits flux only in this case so minus 1 from no. of pixels in cutout for dof
                                result_tab["dof"] = (fitbox**2) - 1
                            else:
                                # psf fits flux, x and y, so minus 3 from no. of pixels in cutout for dof
                                result_tab["dof"] = (fitbox**2) - 3
                            ...
                            t = Time(f["INT_TIMES"].data["int_mid_BJD_TDB"]
                                     [i], format="mjd", scale="tdb")
                            exptime = (
                                f["INT_TIMES"].data["int_end_BJD_TDB"] -
                                f["INT_TIMES"].data["int_start_BJD_TDB"]
                            )[i] * u.d
                            ...
                            if ap_result is None:
                                ...
                                row = [ap_out[0][c]
                                       for c in ap_out.colnames[1:]]
                                ap_result.add_row(
                                    [i+1, t.mjd, exptime.to(u.s), fname] + row)
                            ...
                            result_tab.remove_columns(
                                ['id', 'group_id', 'local_bkg', 'npixfit', 'group_size', 'qfit', 'cfit', 'flags'])
                    ...
                    red_chi2 = np.array(
                        [chi / dof for chi, dof in zip(psf_chi, psf_dof)])
                    ...
                    psf_frame = psf_frame[np.where(
                        red_chi2 < 1.0 if wave_type == 'sw' else red_chi2 < 2.0)]
                    psf_fits_file_name = psf_fits_file_name[np.where(
                        red_chi2 < 1.0 if wave_type == 'sw' else red_chi2 < 2.0)]
                    ...
                    time_second = convert_time_units(
                        np.array(time_mjd), 'second')
                    time_minute = convert_time_units(
                        np.array(time_mjd), 'minute')
                    ...
                    customdata_time = np.array(
                        [{'mjd': mjd, 'phase': phase} for mjd, phase in zip(time_mjd, phase_values)])

                    sorted_indices = sorted(
                        range(len(phase)), key=lambda k: phase[k].value % 1)
                    ...
                    psf_flux_unc_sorted = [psf_flux_unc[i]
                                           for i in sorted_indices]
                    frame_sorted = [frame[i] for i in sorted_indices]
                    fits_file_name_sorted = [fits_file_name[i]
                                             for i in sorted_indices]
                    customdata_phase = np.array([{'mjd': mjd, 'phase': phase} for mjd, phase in zip(
                        time_sorted, phase_values_sorted)])
                    ...
                    # }
                    customdata_time_json = np.array(
                        [json.dumps(item) for item in customdata_time], dtype=str)
                    customdata_phase_json = np.array(
                        [json.dumps(item) for item in customdata_phase], dtype=str)
                    computed_psf = {
                    ...
                    ap_result.write(
                        f"T:\JWST-Timing\jwst\output_with_AfterJay_Recalculated30\Shaw_ZTF_J1539_photometry.AfterJayAndRecalculatedCentroid.{epoch}.{wave_type}.{r_in}.{r_out}.hdf5",
                        overwrite=True, serialize_meta=True, path="aperture"
                    )
                    psf_result.write(
                        f"T:\JWST-Timing\jwst\output_with_AfterJay_Recalculated30\Shaw_ZTF_J1539_photometry.AfterJayAndRecalculatedCentroid.{epoch}.{wave_type}.{r_in}.{r_out}.hdf5",
                        append=True, overwrite=True, serialize_meta=True, path="psf"
                    )
                    computed_psf_table.write(
                        f"T:\JWST-Timing\jwst\output_with_AfterJay_Recalculated30\Shaw_ZTF_J1539_photometry.AfterJayAndRecalculatedCentroid.{epoch}.{wave_type}.{r_in}.{r_out}.hdf5",
                        append=True, overwrite=True, serialize_meta=True, path="computed_psf"
                    )

                    print("Time taken to run (s):", time.process_time())

centroid_csv_path = r"T:\JWST-Timing\jwst\output_with_AfterJay_Recalculated30\centroid_log.csv"

with open(centroid_csv_path, "w", newline="") as csvfile:
    fieldnames = ["fits_file", "ap_x", "ap_y", "psf_x", "psf_y"]
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(centroid_log)

print(f"Saved centroid CSV: {centroid_csv_path}")
