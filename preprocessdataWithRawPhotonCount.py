...
def flux_to_raw_photons(flux_mjy_sr, flux_err_mjy_sr, photmjsr, effinttm):
    """
    Convert calibrated flux (MJy/sr) back to raw electron (photon) count.

    Background:
    -----------
    The SCI extension stores flux in MJy/sr — this is the FINAL calibrated unit
    after the full JWST pipeline has run (dark subtraction, flat field, ramp fitting, etc.).

    To get back to raw photon counts, we reverse two calibration steps:

    Step 1 — Undo the photometric calibration (MJy/sr → counts/s):
        PHOTMJSR (from SCI header = 4.474) tells us:
        "1 count/s from this detector produces 4.474 MJy/sr of flux"
        So:  counts_per_sec = flux_MJy_sr / PHOTMJSR

    Step 2 — Convert count rate to total counts over one integration:
        EFFINTTM (from PRIMARY header = 5.294 s) is the effective integration time
        per frame (how long the detector actually collected photons).
        So:  raw_electrons = counts_per_sec x EFFINTTM

    Note on gain:
    -------------
    Normally you would also divide by gain (e-/ADU) to convert ADU → electrons.
    But in this file, S_GANSCL = SKIPPED in the pipeline log, which means
    the gain scale correction was already applied during ramp fitting (S_RAMP = COMPLETE).
    The output is therefore already in electrons, so NO extra gain multiplication needed.

    Parameters
    ----------
    flux_mjy_sr     : float — PSF flux in MJy/sr  (flux_recalc from psf_result)
    flux_err_mjy_sr : float — uncertainty on flux in MJy/sr  (flux_recalc_unc)
    photmjsr        : float — from SCI header PHOTMJSR  (MJy/sr per count/s)
    effinttm        : float — from PRIMARY header EFFINTTM  (seconds per integration)

    Returns
    -------
    raw_electrons     : float — estimated raw electron count for this frame
    raw_electrons_err : float — propagated uncertainty on the electron count
    """

    # Step 1: undo photometric calibration → get count rate (electrons/s)
    # PHOTMJSR = 4.474 MJy/sr per (electron/s)
    # Dividing flux by PHOTMJSR gives us how many electrons/s produced that flux
    cps     = flux_mjy_sr     / photmjsr   # electrons/s
    cps_err = flux_err_mjy_sr / photmjsr   # uncertainty in electrons/s

    # Step 2: multiply by integration time → get total electrons in this frame
    # EFFINTTM = 5.294 s — the actual time the detector collected photons
    # (excludes resets and dropped frames between groups)
    raw_electrons     = cps     * effinttm  # total electrons collected
    raw_electrons_err = cps_err * effinttm  # propagated uncertainty

    return raw_electrons, raw_electrons_err
            ...
            for rOut in range(30, 65, 10):
                if rOut > rIn:
                    ...
                    for x in range(len(fileList)):
                        ...
                        # NEW: read calibration keywords for raw photon conversion
                        photmjsr = f["SCI"].header["PHOTMJSR"]  # MJy/sr per count/s
                        effinttm = f[0].header["EFFINTTM"]       # effective integration time (s)
                        ...
                        for i in range(istart,iend):
                            ...
                            # NEW: raw photon count from aperture photometry
                            ap_raw, ap_raw_err = flux_to_raw_photons(
                                float(ap_out["counts_bkgsub"][0]), float(ap_out["counts_bkgsub_err"][0]),
                                photmjsr, effinttm)
                            ...
                            # NEW: raw photon count from PSF photometry
                            psf_raw, psf_raw_err = flux_to_raw_photons(float(flux), float(flux_err), photmjsr, effinttm)
                            ...
                            if ap_result is None:
                                ...
                                ap_result.add_row([i+1, t.mjd, exptime.to(u.s), fname]  + row + [ap_raw, ap_raw_err])
                            ...
                            if psf_result is None:
                                ...
                                psf_result.add_row([i+1, t.mjd, exptime.to(u.s), fname]  + row + [psf_raw, psf_raw_err])
                    ...
                    psf_raw_counts = np.array(psf_result["raw_photon_count"])        # NEW
                    psf_raw_counts_err = np.array(psf_result["raw_photon_count_err"])# NEW
                    ...
                    psf_raw_counts = psf_raw_counts[np.where(
                        red_chi2 < 1.0 if wave_type == 'sw' else red_chi2 < 2.0)]  # NEW
                    psf_raw_counts_err = psf_raw_counts_err[np.where(
                        red_chi2 < 1.0 if wave_type == 'sw' else red_chi2 < 2.0)]  # NEW
                    ...
                    raw_sorted = [psf_raw_counts[i] for i in sorted_indices]          # NEW
                    raw_unc_sorted = [psf_raw_counts_err[i] for i in sorted_indices]  # NEW
                    ...
                    computed_psf = {
                        ...
                        "raw_photon_count": np.array(psf_raw_counts),          # NEW
                        "raw_photon_count_err": np.array(psf_raw_counts_err),  # NEW
                        ...
                        "raw_photon_count_sorted": np.array(raw_sorted),          # NEW
                        "raw_photon_count_unc_sorted": np.array(raw_unc_sorted),  # NEW
                    ...
                    combined_dict = {
                        ...
                        "aperture": {
                                    ...
                                    "raw_photon_count": ap_result["raw_photon_count"].tolist(),      # NEW
                                    "raw_photon_count_err": ap_result["raw_photon_count_err"].tolist(),  # NEW
                        },
                        "psf": {
                                ...
                                "raw_photon_count": psf_result["raw_photon_count"].tolist(),      # NEW
                                "raw_photon_count_err": psf_result["raw_photon_count_err"].tolist(),  # NEW
                        },
                        "computed_psf": {
                            ...
                            "raw_photon_count": computed_psf_table["raw_photon_count"].tolist(),          # NEW
                            "raw_photon_count_err": computed_psf_table["raw_photon_count_err"].tolist(),  # NEW
                            ...
                            "raw_photon_count_sorted": computed_psf_table["raw_photon_count_sorted"].tolist(),          # NEW
                            "raw_photon_count_unc_sorted": computed_psf_table["raw_photon_count_unc_sorted"].tolist(),  # NEW
                    ...
                    ap_result.write(
                        f"T:\JWST-Timing\jwst-idvl\jwst-data-raw-photon-count\hdf5\Shaw_ZTF_J1539_photometry."+str(epoch)+"."+str(wave_type)+"."+str(r_in)+"."+str(r_out)+".hdf5", overwrite=True, serialize_meta=True, path="aperture"
                    )

                    psf_result.write(
                        f"T:\JWST-Timing\jwst-idvl\jwst-data-raw-photon-count\hdf5\Shaw_ZTF_J1539_photometry."+str(epoch)+"."+str(wave_type)+"."+str(r_in)+"."+str(r_out)+".hdf5", append=True, overwrite=True, serialize_meta=True, path="psf"
                    )

                    computed_psf_table.write(
                        f"T:\JWST-Timing\jwst-idvl\jwst-data-raw-photon-count\hdf5\Shaw_ZTF_J1539_photometry."+str(epoch)+"."+str(wave_type)+"."+str(r_in)+"."+str(r_out)+".hdf5", append=True, overwrite=True, serialize_meta=True, path="computed_psf"
                    )
