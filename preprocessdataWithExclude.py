...
exclusion_set_list = {}
exclusion_file_list = ['outlier_sigma3.csv', 'outlier_sigma2_5.csv', 'outlier_sigma2.csv', 'outlier_sigma1_5.csv']

for exclusion_file in exclusion_file_list:
    filepath = f'./input/{exclusion_file}'
    if os.path.exists(filepath):
        excl_df = pd.read_csv(filepath)
        # Load exclusion as 0-indexed by subtracting 1
        exclusion_set = set(zip(excl_df['fits_file'], excl_df['slice_idx'].astype(int) - 1))
        exclusion_set_list[exclusion_file] = exclusion_set  # use filename string as key
        print(f"Loaded {exclusion_file}: {len(exclusion_set)} frames to exclude")
    else:
        print(f"File not found, skipping: {filepath}")

# print(exclusion_set_list)
for exclusion_file, exclusion_set in exclusion_set_list.items():
    for epoch in ["epoch1"]:
                ...
                # for rOut in range(30, 65, 10):
                    if rOut > rIn:
                        ...
                        for x in range(len(fileList)):
                            ...
                            fits_stem = os.path.basename(filename).replace('.fits', '')
                            good_frames = [
                                i for i in range(istart, iend)
                                if (fits_stem, i) not in exclusion_set
                            ]
                            print(f"Using {len(good_frames)}/{iend} frames after exclusion")
                            meandata = np.nanmean(data.data[good_frames], axis=0)

                            # store results with exclusion_file in the id
                            id = f"{exclusion_file}_{epoch}_{wave_type}_{rIn}_{rOut}"
                        ...
                        excl_label = exclusion_file.replace('.csv', '')

                        ap_result.write(
                            f"T:\JWST-Timing\jwst\output_with_exclude\Shaw_ZTF_J1539_photometry.{excl_label}.{epoch}.{wave_type}.{r_in}.{r_out}.hdf5",
